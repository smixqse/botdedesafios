...
exports.run = (bot, message, args) => {
  ...
  if (args.length < 1) {
    reply(
      `Coloque entre <> uma "menção" para o canal, cargo ou usuário que você quer substituir no texto, mesmo que o Discord não sugira. Ex.: \`${bot.config.prefix}format Entre no canal <#canal> se tiver o cargo <@Cargo Loko Demais>\``
    );
  } else {
    var text = args.join(" ");
    while (text.indexOf("<") !== -1) {
      ...
      if (!["@", "#"].includes(contents[0])) {
        ...
        if (!isNaN(contents.slice(2)) && contents.length > 15) {
          ...
          if (contents[0] == "@") {
            var roleSearch = message.guild.roles.filter((a) =>
              a.name.toLowerCase().includes(contents.slice(1).toLowerCase())
            );
            ...
            if (!isNaN(contents.slice(2))) {
              ...
              var channelSearch = message.guild.channels.filter(
                (a) => a.name == contents.slice(1)
              );
    ...
    var embed = new Discord.MessageEmbed()
      .setColor(message.guild.me.roles.color.hexColor)
      .setTitle("Resultado")
      .setDescription(`\`\`\`${Discord.escapeMarkdown(text, true)}\`\`\``);
